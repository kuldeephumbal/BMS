...
import { FaPlus, FaEdit, FaTrash, FaSearch, FaTimes, FaRupeeSign } from 'react-icons/fa';
...
export default function Service() {
    ...
    const handleSubmit = (e) => {
        ...
        if (editingService) {
            // For updates, handle image separately if needed
            const formData = new FormData();
            formData.append('name', form.name.trim());
            formData.append('price', parseFloat(form.price));
            formData.append('unit', form.unit.trim());
            formData.append('includeTax', form.includeTax);
            formData.append('SACCode', form.SACCode.trim());
            formData.append('GST', form.GST.trim());
            ...
            axios.put(`${BASE_URL}/service/${editingService._id}`, formData, config)
                .then(res => {
            ...
            // For creation, use FormData to properly upload the image
            const formData = new FormData();
            ...
            axios.post(`${BASE_URL}/service`, formData, config)
                .then(res => {
                        ...
                        <div className="main-data-header-row">
                            <div className="d-flex align-items-center justify-content-between w-100">
                                <div className="d-flex align-items-center gap-3">
                                    <h1 className="main-data-page-title d-flex align-items-center gap-2">Services</h1>
                            ...
                            <table className="main-data-table product-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th style={{ textAlign: 'left' }}>SAC</th>
                                        <th style={{ textAlign: 'left' }}>GST</th>
                                        <th style={{ textAlign: 'left' }}>TAX INCL.</th>
                                ...
                                <tbody>
                                    {loading ? (<tr><td colSpan="8" className="text-center py-4">Loading services...</td></tr>) : filteredServices.length === 0 ? (<tr><td colSpan="8" className="text-center py-4">{search ? 'No services match your search.' : 'No services found. Add your first service!'}</td></tr>) : (filteredServices.map(service => (
                                        <tr key={service._id} className="main-data-table-row">
                                            ...
                                            <td style={{ textAlign: 'left' }}>{service.image ? (<img src={getImageUrl(service.image)} alt={service.name} style={{ width: 40, height: 40, objectFit: 'cover', borderRadius: 6, border: '1px solid #eee' }} />) : <span className="text-muted">No Image</span>}</td>
                                            <td style={{ textAlign: 'left' }}>{service.unit}</td>
                                            <td className="fw-bold" style={{ textAlign: 'right' }}>{formatCurrency(service.price)}</td>
                                            <td style={{ textAlign: 'left' }}>{service.SACCode}</td>
                                            <td style={{ textAlign: 'left' }}>{service.GST}</td>
                                            <td style={{ textAlign: 'left' }}>{service.includeTax ? 'Yes' : 'No'}</td>
                                            <td><div className="action-buttons"><button className="btn-edit" onClick={() => handleEdit(service)} title="Edit Service"><FaEdit /></button><button className="btn-delete-icon" onClick={() => handleDelete(service._id)} title="Delete Service"><FaTrash /></button></div></td>
                                        </tr>)))}
                                </tbody>
                            </table>
                        </div>
                        {modalOpen && (<div className="main-data-modal-backdrop">
                            <div className="main-data-modal" style={{ maxWidth: 720 }}>
                                <div className="main-data-modal-header">
                                    <h2 className="main-data-modal-title">{editingService ? 'Edit Service' : 'Add New Service'}</h2>
                                ...
                                <form onSubmit={handleSubmit} className="main-data-modal-form">
                                    ...
                                    <div className="main-data-modal-row">
                                        <label className="main-data-modal-label" style={{ flex: 1 }}>Active Business
                                            <input type="text" value={activeBusiness?.business_name || 'Active Business'} className="main-data-modal-input" disabled />
                                        </label>
                                        <label className="main-data-modal-label" style={{ flex: 1 }}>Name *
                                            <input type="text" name="name" value={form.name} onChange={handleChange} className="main-data-modal-input" placeholder="Service name" required />
                                        </label>
                                        <label className="main-data-modal-label" style={{ flex: 1 }}>Unit *
                                            <input type="text" name="unit" value={form.unit} onChange={handleChange} className="main-data-modal-input" placeholder="e.g., hrs" required />
                                        </label>
                                    </div>
                                    <div className="main-data-modal-row">
                                        <label className="main-data-modal-label" style={{ flex: 1 }}>Price *
                                            <div className="d-flex align-items-center gap-2">
                                                <span className="text-muted"><FaRupeeSign size={14} /></span>
                                                <input type="number" name="price" value={form.price} onChange={handleChange} className="main-data-modal-input" placeholder="0" min="0" step="0.01" required />
                                            </div>
                                        </label>
                                        <label className="main-data-modal-label" style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 8 }}>
                                            <input type="checkbox" name="includeTax" checked={form.includeTax} onChange={handleChange} style={{ width: 18, height: 18 }} /> Tax Included
                                        </label>
                                        <label className="main-data-modal-label" style={{ flex: 1 }}>Image
                                    ...
                                    <div className="main-data-modal-row">
                                        <label className="main-data-modal-label" style={{ flex: 1 }}>SAC Code *
                                            <input type="text" name="SACCode" value={form.SACCode} onChange={handleChange} className="main-data-modal-input" placeholder="SAC Code" required />
                                        </label>
                                        <label className="main-data-modal-label" style={{ flex: 1 }}>GST *
                                            <input type="text" name="GST" value={form.GST} onChange={handleChange} className="main-data-modal-input" placeholder="GST" required />
                                        </label>
                                    </div>
                                    <div className="main-data-modal-actions">
                                        ...
                                        <button type="submit" className="btn-save">{editingService ? 'Update Service' : 'Add Service'}</button>
                                    </div>
                                </form>
                            </div>
                        </div>)}
